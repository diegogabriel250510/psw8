...
def gerar_pdf_exames(exame,paciente, senha):
    path_templates = os.path.join(settings.BASE_DIR, 'templates/partials/senha_exame.html')
    template_render = render_to_string(path_templates, {'exame':exame, 'paciente':paciente, 'senha':senha})
    path_outut = BytesIO()
    
    HTML(string=template_render).write_pdf(path_outut)
    path_outut.seek(0)
    return path_outut