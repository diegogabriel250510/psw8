...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(request, constants.ERROR, "esse pedido não é seu, portanto voce não pode cancelado")
        return redirect('/exames/gerenciar_pedidos')
...
def solicitar_senha_exame(request, exame_id):
    ...
    if request.method == 'GET':
        ...
        if senha == exame.senha:
            ...
            messages.add_message(request,constants.ERROR, 'Senha invallida')
...
def acesso_medico(request,token):
    ...
    if acesso_medico.status == 'Expirado':
        messages.add_message(request, constants.ERROR, 'Esse token ja expirou, solicite outro')
